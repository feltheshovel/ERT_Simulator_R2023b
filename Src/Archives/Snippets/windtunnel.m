...
for v=[20,30,40,50,60,70,80]
    ...
    C_D = [C_D ; c_D];
    f_Dab = 0.5*c_Dab*rho*pi*Rocket.maxDiameter^2/4*v^2;
    F_Dab = [F_Dab ; f_Dab];
    f_D = 0.5*c_D*rho*pi*Rocket.maxDiameter^2/4*v^2;
    F_D = [F_D ; f_D];
    c_Na = normalLift(Rocket, alpha, K, M, 0, 1);
    C_Na = [C_Na ; c_Na];
    f_N = 0.5*c_Na*alpha*rho*pi*Rocket.maxDiameter^2/4*v^2;
    F_N = [F_N ; f_N];
...
% F_Dab = 0.5*C_Dab*rho*pi*Rocket.maxDiameter^2/4*V_inf^2
% F_D = 0.5*C_D*rho*pi*Rocket.maxDiameter^2/4*V_inf^2
% C_Na = normalLift(Rocket, alpha, K, M, 0, 1)
% F_N = 0.5*C_Na*alpha*rho*pi*Rocket.maxDiameter^2/4*V_inf^2
